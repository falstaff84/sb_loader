...
--*/
{
	...
	for (DevType = MX23; DevType < NoDev; DevType += 1)
	{
		...
		case MX6Q:
			...
			break;
		case MX6D:
			filter.Format(_T("%s#vid_%04x&pid_%04x"), _T("HID"), FSL_VID, MX6D_USB_PID);
			break;
		case MX6SL:
			filter.Format(_T("%s#vid_%04x&pid_%04x"), _T("HID"), FSL_VID, MX6SL_USB_PID);
			break;
		case MX6SX:
			filter.Format(_T("%s#vid_%04x&pid_%04x"), _T("HID"), FSL_VID, MX6SX_USB_PID);
			break;
		case MX7D:
			filter.Format(_T("%s#vid_%04x&pid_%04x"), _T("HID"), FSL_VID, MX7D_USB_PID);
			break;
		case MX6UL:
		...
		case MX6ULL:
			...
			break;
		case TRDX_SDP:
			filter.Format(_T("%s#vid_%04x"), _T("HID"), 0x1b67);
			break;
		default:
